...
public class CameraReactions extends LinearOpMode {
    ...
    public static class Offset {
        ...
        public Offset(HardwareMap hardwareMap, Pose2d pose) {
             ...
             */
            SampleDetectionSupreme supreme = new SampleDetectionSupreme();
            rPipeline = supreme.new RedObjectPipeline();
            yPipeline = supreme.new YellowObjectPipeline();
            bPipeline = supreme.new BlueObjectPipeline();
        ...
        public class YOffsetRedSide implements Action {
            ...
            public boolean run(@NonNull TelemetryPacket telemetryPacket) {
                ...
                if (Math.abs(yOffset) < 10) { //if its in range, don't move
                    tickerY++;
                    if (tickerY >= checker) { //only consider it done when its been in range for 5 calcs
